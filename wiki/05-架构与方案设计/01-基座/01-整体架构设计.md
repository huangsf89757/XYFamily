...
> XYFamily 多租户账号权限底座整体技术架构。本文档是「05-架构与方案设计」的 **P1 基座文档**，基于产品 PRD（多租户底座 V1.0.0）重新梳理，定义技术栈、分层架构、模块边界、部署拓扑、请求处理链路总览与关键设计约束基线，为后续核心域方案（多租户隔离 / RBAC / JWT）、数据模型、接口契约、链路实现与横切专项提供统一基础。
...
- **与 ADR 的关系**：本文中的关键取舍（单角色 RBAC、应用层隔离、Access 无状态 + Refresh 有状态等）均以 ADR 形式记录在《[ADR架构决策记录](../01-基座/02-ADR架构决策记录.md)》中，本文只给结论与总览，决策依据以 ADR 为准。
- **与 PRD 的关系**：本架构完全服务于产品 PRD（多租户底座），PRD 描述"要做什么（What）"，本文与后续 03 文档描述"怎么做（How）"。PRD 来源：[产品PRD](../../04-需求与产品设计/01-产品PRD/产品PRD.md)。
- **阅读顺序建议**：本文 → ADR → P2 核心域（[XYFamily Wiki - 知识库](../../README.md) / [核心域](../02-核心域/核心域.md) / [核心域](../02-核心域/核心域.md)）→ P3 数据模型与契约（[数据库设计](../03-数据模型与契约/01-数据库设计/数据库设计.md) / [接口设计](../03-数据模型与契约/02-接口设计/接口设计.md)）→ P4 链路实现（[链路实现](../04-链路实现/链路实现.md) / [链路实现](../04-链路实现/链路实现.md) / [链路实现](../04-链路实现/链路实现.md)）→ P5 支撑域 → P6 横切专项 → P7 收口（NFR 落地映射）。
...
| PRD 模块 | 服务模块 | 职责 | 数据模型（见 [数据库设计](../03-数据模型与契约/01-数据库设计/数据库设计.md)） |
|------|------|------|------|
...
| 支撑 | **Notify** | 短信/邮件/站内通知（优雅降级） | 第三方集成（见 [支撑域](../05-支撑域/支撑域.md)） |
...
## 六、多租户隔离总览（详细见 [核心域](../02-核心域/核心域.md)）
...
**部署要点**：服务无状态，可水平扩展与负载均衡（NFR-SCAL-001、NFR-AVAIL-002 无单点）；PG 主从、读写分离预留（NFR-SCAL-002）；Redis 用 Sentinel 保证高可用；非核心依赖（通知）故障时核心认证/管理仍可用（NFR-AVAIL-003 优雅降级）。容灾与备份细节见 [横切专项](../06-横切专项/横切专项.md)。
...
> **说明**：各前端端详细目录结构见分端代码规范对应文档。各端里程碑与交付计划见 [里程碑](../../03-里程碑/里程碑.md)。
...
- [数据库设计](../03-数据模型与契约/01-数据库设计/数据库设计.md) — 全量 PG DDL、枚举、索引、预初始化数据（P3）
- [接口设计](../03-数据模型与契约/02-接口设计/接口设计.md) — 各模块 API 契约（P3）
- [链路实现](../04-链路实现/链路实现.md) — 链路中间件实现细节（P4）
- [核心域](../02-核心域/核心域.md) — 多租户隔离 / RBAC / JWT 落地（P2）
- [链路实现](../04-链路实现/链路实现.md) — 端到端时序图（P4）
- [横切专项](../06-横切专项/横切专项.md) — 高可用与备份（P6）
- [产品PRD](../../04-需求与产品设计/01-产品PRD/产品PRD.md) — 需求来源（What）

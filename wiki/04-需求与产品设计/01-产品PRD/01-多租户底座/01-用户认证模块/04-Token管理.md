...
| 关联目录 | 04-需求与产品设计/01-产品PRD/01-多租户底座/01-用户认证模块 |
...
**并发刷新处理：**
- 采用 Refresh Token 单次使用机制（One-Time Use）：刷新成功返回新的 Token 对，旧 Refresh Token 立即标记为已使用（或删除）
- 通过唯一约束或分布式锁防止并发问题：仅第一个请求成功并返回新 Token 对
- 后续请求若携带旧 Refresh Token（已被使用），返回「Token 已被使用，请重新登录」
- 若后续请求携带新 Refresh Token（已收到新 Token 对），正常返回新的 Access Token（幂等）
- 若旧 Refresh Token 在过期后再次被使用（说明被窃取），触发安全告警，该账号所有会话立即失效
...
| 注册认证 | [注册认证](./01-注册认证.md) | 注册功能详细规格 |
| 登录认证 | [登录认证](./02-登录认证.md) | 登录功能详细规格 |
| 密码管理 | [密码管理](./03-密码管理.md) | 密码管理详细规格 |
...
> 修改密码详见：[密码与安全](../02-账号管理模块/02-密码与安全.md)

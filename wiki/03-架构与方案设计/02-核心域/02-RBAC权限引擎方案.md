...
决策依据：[ADR-002 单角色](../../01-基座/02-ADR架构决策记录.md)、[ADR-007 层级回溯+缓存](../../01-基座/02-ADR架构决策记录.md)、[ADR-008 45 权限点+Public](../../01-基座/02-ADR架构决策记录.md)；约束基线 [C1/C9](../../01-基座/01-整体架构设计.md)。
...
> 命中率目标 ≥ 95%（NFR-PERF-008，详见 [缓存设计方案](../05-支撑域/README.md)）。
...
详细链路见 [中间件链专项方案](../04-链路实现/README.md) 与 [业务流程时序图集](../04-链路实现/README.md)。
...
- [数据库设计](../03-数据模型与契约/01-数据库设计/README.md) — `roles`/`permission_points`/`role_permissions` 落库与预初始化

...
| V1.1.0 | 2026-07-16 | ARCH-004 修复：补充跨存储事务失败处理策略；ARCH-006 修复：引用 ADR-016；ARCH-009 修复：补充闲置超时与异地登录提醒 | CatPaw |
...
- 算法：HS256（首期）→ 可平滑升级 RS256（NFR-SEC-005，升级方案见 [ADR-016](../01-基座/02-ADR架构决策记录.md#adr-016jwt-签名算法升级与密钥轮换方案)）。
...
| [中间件链](../04-链路实现/链路实现.md) | JWTValidator、TokenBlacklist 实现 |
| [审计日志](../04-链路实现/链路实现.md) | 登录/登出/刷新/异常行为审计 |
...
- [Token 管理 PRD](../../02-需求与产品设计/01-产品PRD/01-多租户底座/01-用户认证模块/04-Token管理.md)

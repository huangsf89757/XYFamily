...
6. **SuperAdmin 初始化**：数据库为空时创建首个 SuperAdmin（[ADR-001](../../02-基座/02-ADR架构决策记录.md)，CLI/环境变量）。

> 迁移文件置于 `code/backend/migrations/`（见 [P1 §九](../../02-基座/01-整体架构设计.md)），按版本号升序执行。
...
- 驱动 [P4 中间件链](../../../05-链路实现/01-中间件链专项方案.md) 的隔离校验与权限查询。

## 六、关联文档

- [整体架构设计](../../02-基座/01-整体架构设计.md) — 项目目录、约束基线
- [ADR 架构决策记录](../../02-基座/02-ADR架构决策记录.md) — ADR-001/004/005/006/011

...
> P5 支撑域之一。定义账号从注册到注销、宽限期、匿名化、恢复的全生命周期状态机与定时任务，并明确身份标识体系。承接 [账号认证域库表](../03-数据模型与契约/01-数据库设计/01-账号与认证域.md#二accounts账号主表)、[P2 JWT §七](../02-核心域/03-JWT鉴权链与Token方案.md) 会话规则，依据 ADR-006/010 与账号 PRD（FR-ACCT-004/005）。
...
| 版本 | 日期 | 变更内容 | 变更人 |
|------|------|----------|--------|
| V1.0.0 | 2026-07-15 | 基于账号 PRD 与 ADR 定义生命周期状态机 | ClaudeCode |
| V1.1.0 | 2026-07-16 | ARCH-014 修复：补充登录时匿名化兜底机制 | CatPaw |
...
| 下游 | 本方案提供 |
|------|------------|
| [P2 JWT 会话规则](../../02-核心域/03-JWT鉴权链与Token方案.md) | 注销 → 会话撤销/黑名单 |
| [审计日志方案](../04-链路实现/03-审计日志方案.md) | 注销/恢复审计 + 匿名化 |
| [账号接口](../03-数据模型与契约/02-接口设计/02-账号接口.md) | 状态字段契约 |

## 五、关联文档

- [账号认证域库表](../03-数据模型与契约/01-数据库设计/01-账号与认证域.md)
- [ADR-006 软删除](../01-基座/02-ADR架构决策记录.md) / [ADR-010 注销匿名化](../../01-基座/02-ADR架构决策记录.md)

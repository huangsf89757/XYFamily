...
| 版本 | 日期 | 变更内容 | 变更人 |
|------|------|----------|--------|
| V1.0.0 | 2026-07-15 | 基于 PRD 权限管理模块重新梳理，定义 RBAC 引擎与二维继承/缓存策略 | ClaudeCode |

---

## 一、定位与 PRD 来源

PRD 权限管理模块（[权限管理模块 PRD](../02-需求与产品设计/01-产品PRD/01-多租户底座/06-权限管理模块/权限管理模块-V1.0.0.md)）要求：

- 预定义角色初始化（FR-PERM-001）、请求时权限点校验（FR-PERM-002）、数据范围控制（FR-PERM-003）、权限继承（FR-PERM-004）。
- 仅预定义角色、单角色绑定、二维继承、继承自初始化固化。

决策依据：[ADR-002 单角色](../../02-基座/02-ADR架构决策记录.md)、[ADR-007 层级回溯+缓存](../../02-基座/02-ADR架构决策记录.md)、[ADR-008 45 权限点+Public](../../02-基座/02-ADR架构决策记录.md)；约束基线 [C1/C9](../../02-基座/01-整体架构设计.md)。
...
完整矩阵（认证 5 + 账号 7 + 组织 9 + 团队 9 + 小组 8 + 审计 3 + 超级管理员 4）以 [PRD 权限点矩阵](../02-需求与产品设计/01-产品PRD/01-多租户底座/06-权限管理模块/权限管理模块-V1.0.0.md#五权限点矩阵) 为权威来源，本方案不重复罗列，仅规定其**结构化落地方式**（见第五节）。
...
权限继承为**二维**（PRD [04-权限继承](../02-需求与产品设计/01-产品PRD/01-多租户底座/06-权限管理模块/04-权限继承-V1.0.0.md)）：
...
```mermaid
flowchart LR
    P[Public L0] --> RM[RegularMember L1]
    RM --> GRA[小组普通 L2]
    GRA --> GA[小组核心 L3]
    GA --> TRA[团队普通 L4]
    TRA --> TA[团队核心 L5]
...
| 项目 | 策略 |
|------|------|
| 缓存键 | `perm:account:{account_id}:org:{org_id}`，值为该账号在该组织下的有效权限点集合（含回溯展开后的角色权限） |
| TTL | 24h |
| 写入 | PermissionChecker 未命中时查 DB 并回填；数据范围由 MembershipValidator 先确定 |
| 失效 | 角色/成员关系变更（分配/降级/移除/邀请接受）、权限矩阵重新预填充时，主动删除相关 key |
| 降级 | 缓存不可用时回退 DB 查询（不阻断业务，仅性能下降） |

> 命中率目标 ≥ 95%（NFR-PERF-008，详见 [缓存设计方案](../06-支撑域/README.md)）。
...
- 权限不是全局生效，而是绑定在请求携带的 scope 上下文中（见 [多租户隔离方案 §四/五](./01-多租户隔离方案.md)）。
...
详细链路见 [中间件链专项方案](../05-链路实现/README.md) 与 [业务流程时序图集](../05-链路实现/README.md)。

---

## 九、关联文档

- [整体架构设计](../../02-基座/01-整体架构设计.md) — 约束基线 C1/C7/C9、请求处理链路
- [ADR 架构决策记录](../../02-基座/02-ADR架构决策记录.md) — ADR-002/007/008
- [多租户隔离方案](./01-多租户隔离方案.md) — 上下文校验与层级回溯
- [JWT 鉴权链与 Token 方案](./03-JWT鉴权链与Token方案.md) — Token Claims 中的 `roles`
- [数据库设计](../04-数据模型与契约/01-数据库设计/README.md) — `roles`/`permission_points`/`role_permissions` 落库与预初始化
- [权限管理模块 PRD](../02-需求与产品设计/01-产品PRD/01-多租户底座/06-权限管理模块/权限管理模块-V1.0.0.md)

...
决策依据：[ADR-003 Session 与 JWT 共存](../../02-基座/02-ADR架构决策记录.md)；约束基线 [C3/C6](../../02-基座/01-整体架构设计.md)。
...
| 下游方案 | 本方案提供什么 |
|----------|----------------|
| [数据库设计](../04-数据模型与契约/01-数据库设计/README.md) | `sessions`（Refresh 有状态）、`token_blacklist` 结构 |
| [接口设计](../04-数据模型与契约/02-接口设计/README.md) | 登录/刷新/登出/改密契约与错误码 |
| [中间件链](../05-链路实现/README.md) | JWTValidator、TokenBlacklist 实现 |
| [审计日志](../05-链路实现/README.md) | 登录/登出/刷新/异常行为审计 |

---

## 十、关联文档

- [整体架构设计](../../02-基座/01-整体架构设计.md) — 约束基线 C3/C6、请求处理链路
- [ADR 架构决策记录](../../02-基座/02-ADR架构决策记录.md) — ADR-003

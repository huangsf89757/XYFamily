...
- **阅读顺序建议**：本文 → ADR → P2 核心域（[03-多租户隔离](../../README.md) / [RBAC](../02-核心域/核心域.md) / [JWT](../02-核心域/核心域.md)）→ P3 数据模型与契约（[04-数据库设计](../03-数据模型与契约/01-数据库设计/数据库设计.md) / [04-接口设计](../03-数据模型与契约/02-接口设计/接口设计.md)）→ P4 链路实现（[05-中间件链](../04-链路实现/链路实现.md) / [05-业务流程时序图集](../04-链路实现/链路实现.md) / [05-审计日志](../04-链路实现/链路实现.md)）→ P5 支撑域 → P6 横切专项 → P7 收口（NFR 落地映射）。
...
| PRD 模块 | 服务模块 | 职责 | 数据模型（见 [04-数据库设计](../03-数据模型与契约/01-数据库设计/数据库设计.md)） |
|------|------|------|------|
...
| 支撑 | **Notify** | 短信/邮件/站内通知（优雅降级） | 第三方集成（见 [06-第三方集成统一方案](../05-支撑域/支撑域.md)） |
...
## 六、多租户隔离总览（详细见 [02-核心域](../02-核心域/核心域.md)）
...
**部署要点**：服务无状态，可水平扩展与负载均衡（NFR-SCAL-001、NFR-AVAIL-002 无单点）；PG 主从、读写分离预留（NFR-SCAL-002）；Redis 用 Sentinel 保证高可用；非核心依赖（通知）故障时核心认证/管理仍可用（NFR-AVAIL-003 优雅降级）。容灾与备份细节见 [06-横切专项 / 容灾多活与高可用](../06-横切专项/横切专项.md)。
...
> **说明**：各前端端详细目录结构见分端代码规范对应文档。各端里程碑与交付计划见 [项目里程碑](../../01-项目总览/03-里程碑/里程碑.md)。
...
- [中间件专项方案](../04-链路实现/链路实现.md) — 链路中间件实现细节（P4）
- [核心域方案](../02-核心域/核心域.md) — 多租户隔离 / RBAC / JWT 落地（P2）
- [业务流程时序图集](../04-链路实现/链路实现.md) — 端到端时序图（P4）
- [容灾多活与高可用](../06-横切专项/横切专项.md) — 高可用与备份（P6）

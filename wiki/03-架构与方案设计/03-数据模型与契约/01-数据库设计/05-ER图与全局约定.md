...
| V1.1.0 | 2026-07-16 | ARCH-001 修复：更新 PII 加密约定为盲索引方案 | CatPaw |
| V1.2.0 | 2026-07-19 | 移除 P1 init 链接末尾多余空格，使其解析为有效目录 | CodeBuddy |
...
| PII 加密 | phone/email 采用「盲索引（Blind Index）」方案：密文存 `phone_encrypted`/`email_encrypted`（AES-256-GCM），盲索引存 `phone_hash`/`email_hash`（HMAC-SHA256），登录等值查询走盲索引命中 B-Tree | NFR-SEC-009、ARCH-001 修复 |
...
4. **权限映射预填充**：按 [权限域](./03-权限域.md) 固化展开写入 `role_permissions`。
5. **系统配置种子**：写入 `system_configs` 默认项（[权限域](./03-权限域.md)）。
6. **SuperAdmin 初始化**：数据库为空时创建首个 SuperAdmin（[ADR架构决策记录](../../01-基座/02-ADR架构决策记录.md)，CLI/环境变量）。

> 迁移文件置于 `code/backend/migrations/`（见 [整体架构设计](../../01-基座/01-整体架构设计.md)），按版本号升序执行。
...
- 本文件汇总 [账号与认证域](./01-账号与认证域.md) / [租户域](./02-租户域.md) / [权限域](./03-权限域.md) / [审计域](./04-审计域.md)。
- 驱动 [XYFamily Wiki - 知识库](../../../README.md) 的字段契约与错误码。
- 驱动 [中间件链专项方案](../../04-链路实现/01-中间件链专项方案.md) 的隔离校验与权限查询。

## 六、关联文档

- [整体架构设计](../../01-基座/01-整体架构设计.md) — 项目目录、约束基线
- [ADR架构决策记录](../../01-基座/02-ADR架构决策记录.md) — ADR-001/004/005/006/011
- [核心域](../../02-核心域/核心域.md) — 隔离 / RBAC / JWT

...
- **与 ADR 的关系**：本文中的关键取舍（单角色 RBAC、应用层隔离、Access 无状态 + Refresh 有状态等）均以 ADR 形式记录在《[ADR 架构决策记录](../02-基座/02-ADR架构决策记录.md)》中，本文只给结论与总览，决策依据以 ADR 为准。
- **与 PRD 的关系**：本架构完全服务于产品 PRD（多租户底座），PRD 描述"要做什么（What）"，本文与后续 03 文档描述"怎么做（How）"。PRD 来源：[产品 PRD 总览](../../02-需求与产品设计/01-产品PRD/产品PRD-V1.0.0.md)。
- **阅读顺序建议**：本文 → ADR → P2 核心域（[03-多租户隔离](../03-核心域/README.md) / [RBAC](../03-核心域/README.md) / [JWT](../03-核心域/README.md)）→ P3 数据模型与契约（[04-数据库设计](../04-数据模型与契约/01-数据库设计/README.md) / [04-接口设计](../04-数据模型与契约/02-接口设计/README.md)）→ P4 链路实现（[05-中间件链](../05-链路实现/README.md) / [05-业务流程时序图集](../05-链路实现/README.md) / [05-审计日志](../05-链路实现/README.md)）→ P5 支撑域 → P6 横切专项 → P7 收口（NFR 落地映射）。
...
| 微信小程序 | 原生 / uni-app | 微信生态小程序（微信登录延后 P2，ADR-015） |
| Web | React + TypeScript + Ant Design 5.x | PC 管理后台（对应 `xyfamily-admin-design/`） |
...
| PRD 模块 | 服务模块 | 职责 | 数据模型（见 [04-数据库设计](../04-数据模型与契约/01-数据库设计/README.md)） |
|------|------|------|------|
...
| 支撑 | **Notify** | 短信/邮件/站内通知（优雅降级） | 第三方集成（见 [06-第三方集成统一方案](../06-支撑域/README.md)） |
...
## 六、多租户隔离总览（详细见 [03-核心域](../03-核心域/README.md)）
...
**部署要点**：服务无状态，可水平扩展与负载均衡（NFR-SCAL-001、NFR-AVAIL-002 无单点）；PG 主从、读写分离预留（NFR-SCAL-002）；Redis 用 Sentinel 保证高可用；非核心依赖（通知）故障时核心认证/管理仍可用（NFR-AVAIL-003 优雅降级）。容灾与备份细节见 [07-横切专项 / 容灾多活与高可用](../07-横切专项/README.md)。

---

## 八、关键设计约束基线

以下约束贯穿全部 03 文档，决策依据见《[ADR 架构决策记录](../02-基座/02-ADR架构决策记录.md)》。
...
- [ADR 架构决策记录](../02-基座/02-ADR架构决策记录.md) — 关键架构决策与依据
- [数据库设计](../04-数据模型与契约/01-数据库设计/README.md) — 全量 PG DDL、枚举、索引、预初始化数据（P3）
- [接口设计](../04-数据模型与契约/02-接口设计/README.md) — 各模块 API 契约（P3）
- [中间件专项方案](../05-链路实现/README.md) — 链路中间件实现细节（P4）
- [核心域方案](../03-核心域/README.md) — 多租户隔离 / RBAC / JWT 落地（P2）
- [业务流程时序图集](../05-链路实现/README.md) — 端到端时序图（P4）
- [容灾多活与高可用](../07-横切专项/README.md) — 高可用与备份（P6）
- [产品 PRD 总览](../../02-需求与产品设计/01-产品PRD/产品PRD-V1.0.0.md) — 需求来源（What）

...
| 关联目录 | 02-需求与产品设计/01-产品PRD/01-多租户底座 |

## 变更记录

| 版本 | 日期 | 变更内容 | 变更人 |
|------|------|----------|--------|
| V1.0.0 | 2026-07-10 | 初始草稿 | CatPaw |
| V1.0.1 | 2026-07-12 | 审查修正：权限点 42→45，角色 5 层→6 层（含 Public 第 0 层），FR-TEAM-006 去除 小组核心管理员（`group_core_admin`） | ClaudeCode |
| V1.0.2 | 2026-07-14 | 合并业务流程图集至本文档第 5 节 | CatPaw |
| V1.0.3 | 2026-07-14 | 重新梳理：删除数据字典、接口需求规范等下游内容，仅保留产品需求（What），不含接口、数据库表等实现细节 | CatPaw |
...
| FR-AUDIT-002 | 操作审计日志 | P1 | 记录关键操作 |
| FR-AUDIT-003 | 审计日志查询 | P1 | 支持按条件查询 |

### 3.8 超级管理员模块

| ID | 需求描述 | 优先级 | 验收标准 |
|----|----------|--------|----------|
| FR-ADMIN-001 | 系统配置管理 | P1 | 查看和修改系统配置 |
| FR-ADMIN-002 | 强制降级 | P1 | 强制降级任何用户角色 |
| FR-ADMIN-003 | 全局审计日志 | P1 | 查看所有组织审计日志 |
